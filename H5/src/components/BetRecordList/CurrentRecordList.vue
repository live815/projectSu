<template>
    <div class="list-content-main BetRecordListview">
      ...
      >
        <!-- 标题badge -->
        <div class="title-box">{{ titleBadgeText(item) }}</div>
        <!-- 已结算注单的输赢状态标识 -->
        <div class="status">
        ...
        <div class="order-no-check">
          ...
          <div class="check">
            ...
            <p class="check-time">{{ formatTime(item.cte) }}</p>
          </div>
          <div class="order-no">
            <p class="order-title">
              <span>{{ $t('common.sportsOrder') }}</span>
            </p>
            <p class="order-num">{{ item.id }}</p>
          </div>
          <div class="copy">
            <div class="line"></div>
            <div class="btn" v-clipboard="item.id">{{ $t('common.copy') }}</div>
          </div>
        </div>
        <!-- 投注额-可赢额 -->
        ...
        <!-- 注单详情 -->
        <div class="game-detail" v-for="itm in item.ops" :key="itm.id">
          <!-- 球类、vs球队 -->
          ...
          <!-- 联赛 -->
          <p class="detail-title-2">{{ itm.ln }}</p>
          <!-- 比赛开始时间 -->
          <p class="detail-title-3">{{ $t('common.beginMatch') }} {{ formatTime(itm.bt) }}</p>
          <!-- 投注项 赔率 盘口类型 -->
          <p class="detail-title-4">
            <span>{{ itm.onm }}</span>
            <span class="m-odd">@{{ itm.bo }}</span>
            
          </p>
          <p class="detail-title-4">
            <span class="m-odd-type"> 
              <span v-if="itm.ip==1">{{ $t('common.sportLive') }}-</span>{{ itm.mgn }}
              <!-- 下注时比分 -->
        ...
        <!-- 折叠 -->
        <div v-if="item.ops?.length > 1" @click="collapse(item, index)" class="collapse">
          <van-button
            style="height: 30px"

<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item :label="$t('operation.Firstdeposit1')" prop="firstDepositTime">
        <el-date-picker
          v-model="firstDpTime"
          value-format="YYYY-MM-DD HH:mm:ss"
          type="datetimerange"
      ...
      <el-form-item :label="$t('operation.Prizecollectime1')" prop="receiveTime">
        <el-date-picker
          v-model="receiveTime"
          value-format="YYYY-MM-DD HH:mm:ss"
          type="datetimerange"
    ...
    <el-table :data="tableDataList" style="width: 100%">
      ...
      <el-table-column prop="firstDpAmount" :label="$t('operation.firstdepositamount')" />
      <el-table-column prop="profitAmount" :label="$t('operation.tip133')" />
...
import { newbieResurrectionGiftRecord } from '@/api/operation/activeConfiguration'
...
const getTableDataList = async () => {
  ...
  try {
    let res = await newbieResurrectionGiftRecord(queryParams)
...
function handleQuery() {
  ...
  if (firstDpTime.value) {
    queryParams.firstDpStartTime = firstDpTime.value[0]
    queryParams.firstDpEndTime = firstDpTime.value[1]
  } else {
    queryParams.firstDpStartTime = null
    queryParams.firstDpEndTime = null
  }
...
function handleExport() {
  ...
  proxy.download(
    '/system/activity/resurrection/gift/record/list/export',
    queryParams,
    `${i18n.global.t('operation.tip139')}-${route.query.name}.xlsx`
  )
}
...
function getList() {
  loading.value = true
  newbieResurrectionGiftRecord(queryParams)
    .then((res) => {
      loading.value = false
      tableDataList.value = res.rows || []
      total.value = res.total

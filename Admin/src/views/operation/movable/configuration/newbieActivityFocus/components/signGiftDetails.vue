<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item :label="$t('operation.Firstdeposit')" prop="firstDepositTime">
        <el-date-picker
          v-model="firstDpTime"
          value-format="YYYY-MM-DD HH:mm:ss"
          type="datetimerange"
    ...
    <el-table :data="tableDataList" style="width: 100%">
      ...
      <el-table-column prop="doneDays" :label="$t('operation.tip134')" />
...
import { newbieSignGiftRecord } from '@/api/operation/activeConfiguration'
...
const getTableDataList = async () => {
  ...
  try {
    let res = await newbieSignGiftRecord(queryParams)
...
function handleExport() {
  ...
  proxy.download(
    '/system/activity/sign/gift/record/list/export',
    queryParams,
    `${i18n.global.t('operation.tip138')}-${route.query.name}.xlsx`
  )
}
...
function getList() {
  loading.value = true
  newbieSignGiftRecord(queryParams)
    .then((res) => {
      loading.value = false
      tableDataList.value = res.rows || []
      total.value = res.total

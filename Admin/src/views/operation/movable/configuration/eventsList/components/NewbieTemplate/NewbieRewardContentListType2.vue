  ...
  <el-form :model="list" ref="ruleFormRef">
    ...
    <el-form-item>
      <el-table :data="list" height="200">
        ...
        <el-table-column prop="depositAmount" :label="$t('newbie.table.label22')" width="140">
          <template #default="{ row }">
            <span v-if="row.depositAmount">{{ row.depositAmount }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column prop="validBet" :label="$t('newbie.table.label23')" width="140">
          <template #default="{ row }">
            <span v-if="row.validBet">{{ row.validBet }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column prop="giftAmount" :label="$t('newbie.table.label20')" width="140">
          <template #default="{ row }">
...
<script setup>
import { ref } from 'vue'
import addInsert from './addNewbieRewardInsertType2.vue'

<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item :label="$t('operation.tip167')" prop="status">
        <el-select
      ...
      <el-form-item :label="$t('operation.tip1600')" prop="auditOneBy">
        <el-input
          v-model="queryParams.auditOneBy"
          :placeholder="$t('operation.pleaseEnter')"
        />
      </el-form-item>
      <el-form-item :label="$t('operation.tip168')" prop="auditTwoBy">
        <el-input
          v-model="queryParams.auditTwoBy"
          :placeholder="$t('operation.pleaseEnter')"
        />
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="carouselList" style="width: 100%">
      <el-table-column :label="$t('operation.operate')" width="120">
        <template #default="scope">
          <el-button
            ...
            >{{ $t("operation.tip169") }}</el-button
          >
        </template>
      </el-table-column>
      <el-table-column
        prop="status"
        :label="$t('operation.tip167')"
        width="200"
      >
        <template #default="scope">
          <el-tag type="warning" v-if="scope.row.status == 0">
            {{ $t("operation.tip170") }}</el-tag
          >
      ...
      <el-table-column
        prop="number"
        :label="$t('operation.Reviewordernumber1')"
        width="280"
      />
      ...
      <el-table-column width="180">
        <template v-slot:header>
          <div class="tab-title">{{ $t("operation.tip162") }}</div>
          <div class="tab-title">{{ $t("operation.tip163") }}</div>
          <div class="tab-title">{{ $t("operation.tip1644") }}</div>
        </template>
        <template #default="scope">
          <div>
            <span>{{ $t("operation.tip165") }}</span>
            <span>{{ scope.row.auditOneBy }}</span>
          </div>
          <div>
            <span>{{ $t("operation.tip166") }}</span>
            <span>{{ scope.row.auditOneTime }}</span>
          </div>
          <div>
            <span>{{ $t("operation.tip164") }}</span>
            <span>{{ scope.row.auditOneRemark }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column width="180">
        <template v-slot:header>
          <div class="tab-title">{{ $t("operation.tip173") }}</div>
          <div class="tab-title">{{ $t("operation.tip174") }}</div>
          <div class="tab-title">{{ $t("operation.tip1644") }}</div>
        </template>
        <template #default="scope">
          <div>
            <span>{{ $t("operation.tip165") }}</span>
            <span>{{ scope.row.auditTwoBy }}</span>
          </div>
          <div>
            <span>{{ $t("operation.tip166") }}</span>
            <span>{{ scope.row.auditTwoTime }}</span>
          </div>
          <div>
            <span>{{ $t("operation.tip164") }}</span>
            <span>{{ scope.row.auditTwoRemark }}</span>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
const queryParams = reactive({
  ...
  number: "", // 审核单号
  operateType: "2", // 0待一审 1一审通过（二审） 2结单查看
  status: "", // 查询审核状态审核状态默认为空
  tenantId: "",
  title: "", // 红利标题
  type: "", // 存入类型
  pageNum: 1,
...
const auditTypes = [
  {
    value: "0",
    label: i18n.global.t("operation.tip170"),
  },
  {
    value: "1",
    label: i18n.global.t("operation.tip171"),
  },
  {
    value: "2",
    label: i18n.global.t("operation.tip172"),
  },
];
const accountTypes = [
  {
...
const getList = async () => {
  const params = {
    status: queryParams.status, // 审核状态

<template>
  <div class="app-container">
    <el-form :model="form" :inline="true">
      <el-form-item :label="$t('operation.VenueType')">
        <el-select
      ...
      <el-form-item>
        ...
        <el-button icon="Plus" type="warning" @click="toShowAdd(true, 2)">{{
          $t("operation.add")
        }}</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-table :data="dataList" style="width: 100%">
      ...
      >
        <template #default="scope">
          <el-tag v-if="scope.row.gameType == 0">{{
            $t("operation.Csports")
          }}</el-tag>
          <el-tag v-if="scope.row.gameType == 1">{{
            $t("operation.FBsports")
          }}</el-tag>
          <el-tag v-if="scope.row.gameType == 2">{{
            $t("operation.DBsports")
          }}</el-tag>
          <!-- <el-tag>{{ scope.row.accountType }}</el-tag> -->
        </template>
      </el-table-column>
      <el-table-column
        prop="title"
...
// 弹窗关闭重新获取数据
const refreshList = (isRefresh = false) => {
  if (isRefresh) {
...
const gameType = [
  ...
  {
    label: i18n.global.t("operation.Csports"),
    value: "0",
  },
  {
    label: i18n.global.t("operation.FBsports"),
    value: "1",
  },
  {
    label: i18n.global.t("operation.DBsports"),
    value: "2",
  },
];
</script>

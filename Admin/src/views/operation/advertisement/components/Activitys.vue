    ...
    <el-row :gutter="20">
      <el-col :span="24" :xs="24">
        <el-form
          ...
          v-show="showSearch"
        >
          <el-form-item :label="$t('operation.Eventtitle')" prop="userName">
            <el-input
              v-model="queryParams.title"
              :placeholder="$t('operation.tip1')"
              clearable
              style="width: 240px"
            />
          </el-form-item>
          ...
          <el-form-item :label="$t('operation.type')" prop="roleId">
            <el-select
        ...
        >
          ...
          >
            <template #default="scope">
              <div v-if="scope.row.activityType === 1">
                {{ $t("operation.Limittime") }}
              </div>
              <div v-if="scope.row.activityType === 2">
                {{ $t("operation.Newcomer") }}
              </div>
              <div v-if="scope.row.activityType === 3">
                {{ $t("operation.daily") }}
              </div>
              <div v-if="scope.row.activityType === 4">
                {{ $t("operation.VIP") }}
              </div>
            </template>
          </el-table-column>
          ...
          >
            <template #default="scope">
              <div v-if="scope.row.activityTag === 1">
                {{ $t("operation.Limittime") }}
              </div>
              <div v-if="scope.row.activityTag === 2">
                {{ $t("operation.Newcomer") }}
              </div>
              <div v-if="scope.row.activityTag === 3">
                {{ $t("operation.daily") }}
              </div>
              <div v-if="scope.row.activityTag === 4">
                {{ $t("operation.VIP") }}
              </div>
              <div v-if="scope.row.activityTag === 5">
                {{ $t("operation.Exclusivesports") }}
              </div>
              <div v-if="scope.row.activityTag === 6">
                {{ $t("operation.Popularevents") }}
              </div>
            </template>
          </el-table-column>
          <el-table-column
...
// 类型
const activityTypeList = reactive([
  {
    value: "",
  ...
  {
    value: "1",
    label: i18n.global.t("operation.Limittime"),
  },
  {
    value: "2",
    label: i18n.global.t("operation.Newcomer"),
  },
  {
    value: "3",
    label: i18n.global.t("operation.daily"),
  },
  {
    value: "4",
    label: i18n.global.t("operation.VIP"),
  },
]);
...
const columns = ref([
  ...
  { key: 2, label: i18n.global.t("operation.status"), visible: true },
  ...
  { key: 10, label: i18n.global.t("operation.founder"), visible: true },
  { key: 11, label: i18n.global.t("operation.creattime"), visible: true },
  {
    key: 12,
    label: i18n.global.t("operation.Mostrecentoperat"),
    visible: true,
  },
  { key: 13, label: i18n.global.t("operation.tip5"), visible: true },
]);
// 表格列表

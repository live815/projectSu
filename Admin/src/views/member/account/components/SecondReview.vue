<template>
  <div class="second-review-dialog">
    <!-- 会员账户修改审核 -->
    ...
    <el-table :data="tableData" :empty-text="$t('member.noData')">
      ...
      <el-table-column :label="$t('member.reviewStatusNoQuote')" align="center" width="120">
        <template #default="{ row }">
          <el-tag v-if="row.status === '4'">{{ $t('member.waitForSecondReview') }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        align="center"
...
import { getSecondReviewList } from '@/api/member/account/index'
...
const checkStatusList = ref([
  { label: i18n.global.t('member.waitForSecondReview'), value: 4 }
])
...
//   if ((dateRangeFirst.value && !dateRangeFirst.value.length) || dateRangeFirst.value == null) {
...
function getList() {
  return getSecondReviewList(queryParams.value).then((res) => {

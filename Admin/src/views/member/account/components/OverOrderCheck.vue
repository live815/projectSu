  ...
  <div class="over-order-check">
    ...
    >
      <el-row :gutter="10">
        ...
        <el-col :span="4">
          <el-form-item :label="$t('member.firstReviewer')" prop="reviewer" class="full-width">
            <el-input
        ...
        <el-col :span="4">
          <el-form-item :label="$t('member.secondReviewer')" prop="finalReview" class="full-width">
            <el-input
    ...
    <el-table :data="tableData" :empty-text="$t('member.noData')">
      ...
      <el-table-column :label="$t('member.appliReasonNoQuote')" align="center" width="200">
        <template #default="{ row }">
          <span>{{ row.createRemake || "-" }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('member.reviewStatusNoQuote')" align="center" width="80">
        <template #default="{ row }">
          <el-tag v-if="row.status === '2'" type="success">{{ $t('member.secondReviewPass') }}</el-tag>
          <el-tag v-if="row.status === '3'" style="color:#F5222D">{{ $t('member.firstReviewReject') }}</el-tag>
          <el-tag v-if="row.status === '5'" style="color:#F5222D">{{ $t('member.secondReviewReject') }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        align="center"
      ...
      >
        <template #default="{ row }">
            <div>
              <span>{{ $t('member.firstReviewerNoQuote') }}:</span>
              <span>{{ row.reviewer || "-" }}</span>
            </div>
            <div>
              <span>{{ $t('member.firstReviewCompleteTimeNoQuote') }}:</span>
              <span>{{ row.reviewTime || "-" }}</span>
            </div>
            <div>
              <span>{{ $t('member.firstReviewRemark') }}:</span>
              <span>{{ row.reviewRemake || "-" }}</span>
            </div>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
            ...
            <div>
              ...
              <span>{{ row.finalReviewRemake || "-" }}</span>
            </div>
        </template>
      </el-table-column>
    </el-table>
...
import { getFinalReviewList } from "@/api/member/account/index";
...
const queryParams = ref({
  ...
  createBy: null, //申请人
  reviewer: null, //一审审核人
  finalReview:null, //二审审核人
  applyId: null, //审核单号
  pageNum: 1,
  pageSize: 10,
});
const checkStatusList = ref([
  { label: i18n.global.t("member.firstReviewReject"), value: 3 },
  { label: i18n.global.t("member.secondReviewReject"), value: 5 },
  { label: i18n.global.t("member.secondReviewPass"), value: 2 },
]);
...
function getList() {
  return getFinalReviewList(queryParams.value).then(res=>{

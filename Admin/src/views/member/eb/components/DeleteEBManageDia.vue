<template>
  <div class="delete-currency-manage-dialog">
    <el-dialog
...
import { delCryptocy } from "@/api/member/bankcard/index";
...
const submit = ()=>{
  queryRef.value.validate(valid=>{
    if(valid){
      delCryptocy(queryParams.value).then(res=>{
        dialogVisible.value = false

        proxy.$modal.msgSuccess("虚拟币地址删除成功");
        emit("onfreshdelete")
      })
    }else{

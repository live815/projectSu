<template>
  <div class="member-running-detail-validate-list" v-loading="loading">
    <!-- 流水已校验列表 -->
    ...
    <el-table :data="tableData" :empty-text="$t('member.noData')">
      ...
      >
        <template #default="{ row }">
          <span>{{
            !isNaN(row.betUnfinished)&&row.betUnfinished ? row.betUnfinished.toFixed(2) : 0
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          <span>{{ filterLabel(manualZeroStatusList,row.manualZeroStatus) }}</span>
        </template>
      </el-table-column>
      <el-table-column
    ...
    <!-- <el-row class="total-table-row" v-if="tableData&&tableData.length!==0">
      <el-col :span="24" class="total-table">
        ...
        <span>
          ...
          <span>{{ totalBetFinished>=totalBetRequest?0:(totalBetRequest-totalBetFinished).toFixed(2) }}</span>
        </span>
...
function getList() {
  loading.value = true;
  return getHistoryList(queryParams.value)
    .then((res) => {
      loading.value = false;
...
.member-running-detail-validate-list {
  .total-table-row{

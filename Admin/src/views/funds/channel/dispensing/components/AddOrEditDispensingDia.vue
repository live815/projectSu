<template>
  <div class="add-edit-dispensing-dialog">
    <el-dialog
      v-model="dialogVisible"
      width="50%"
      :title="type === 'edit' ? $t('funds.editDispensingChannel') : $t('funds.addDispensingChannel')"
      :before-close="cancel"
    ...
    >
      ...
      >
        <el-row :gutter="15">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('funds.singleOrderLimitAmount')" class="full-width" required>
              <el-col :span="11">
          ...
          <el-col v-if="queryParams.channelTypeCode==='USDT'">
          ...
          <el-col :span="12">
            <el-form-item :label="$t('funds.dispensingType')" prop="channelClientName" class="full-width">
              <el-checkbox-group v-model="queryParams.channelClientName">
                <el-checkbox
                  v-for="item in channelClientList"
                  :key="item.value"
                  :label="item.value"
                >
                  {{ item.label }}
...
import { editChannelWithdraw, getDetailWithdraw, addChannelWithdraw } from "@/api/funds/channel/index";
...
const rules = ref({
  ...
  minPay: [
    ...
    {
      pattern: /^\d{1,18}$/,
      message: i18n.global.t("funds.maxEighteenPositiveInteger"),
      trigger: "blur",
    },
  ],
  maxPay: [
...
const channelClientList = ref([
  {
    label:i18n.global.t("funds.member"),
    value:'1'
  },
  {
    label:i18n.global.t("funds.proxy"),
    value:'2'
  }
])
...
//根据ID查询详情
function queryInfo() {
  getDetailWithdraw(props.cloneId).then((res) => {
    if(res){
      res.cryptoProtocolName = res.cryptoProtocol.split(',')
      res.channelClientName = res.channelClient.split(',')
...
const submit = () => {
  queryRef.value.validate((valid) => {
    if (valid) {
      ...
      queryParams.value.channelClient = queryParams.value.channelClientName.join(",");
      if (props.cloneId) {
        editChannelWithdraw(queryParams.value).then((res) => {
          proxy.$modal.msgSuccess(i18n.global.t("funds.editSuccessfully"));
          dialogVisible.value = false;
          emit("onrefresh");
        });
      } else {
        addChannelWithdraw(queryParams.value).then((res) => {

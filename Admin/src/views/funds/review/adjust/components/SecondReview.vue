<template>
  <div class="funds-review-adjust-second-review" v-loading="loading">
    ...
    <el-row :gutter="10" style="margin-bottom: 12px">
      ...
      <el-col :span="10" style="text-align: right">
        ...
        <el-select v-model="multiOperate" :placeholder="$t('funds.pleaseSelect')" clearable>
          <el-option
    ...
    <el-table :data="tableData" @selection-change="handleSelectionChange" :empty-text="$t('funds.noData')">
      ...
      <el-table-column :label="$t('funds.handle')" align="center" width="140">
      ...
      >
        <template #default="{ row }">
          <span style="color: #04F21C">{{
            row.status === "2" ? $t('funds.waitSecondReview') : "-"
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          <template
            v-if="
              row.adjustApplicant ||
              row.adjustApplicantTime ||
              row.adjustApplicantMark
            "
          >
            <div>
            ...
            <div>
              ...
              <span>{{
                row.adjustApplicantTime ? row.adjustApplicantTime : "-"
              }}</span>
            </div>
            <div>
              <span>{{ $t('funds.reorderApplicantMark') }}</span>
              <span>{{
                row.adjustApplicantMark ? row.adjustApplicantMark : "-"
              }}</span>
            </div>
          </template>
          <span v-else>-</span>
...
import { getSecondReviewList } from "@/api/funds/review/index";
import SecondReviewDialog from "./SecondReviewDialog.vue";
...
function getList() {
  loading.value = true;
  return getSecondReviewList(queryParams.value)
    .then((res) => {
      loading.value = false;
...
.funds-review-adjust-second-review {
  .flex-refresh {
    display: flex;

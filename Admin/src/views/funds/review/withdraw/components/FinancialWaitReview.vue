<template>
  <div class="financial-wait-review" v-loading="loading">
    ...
    <el-row :gutter="10">
      ...
      <el-col :span="8" class="flex-refresh">
        <div>
    ...
    <el-table :data="tableData" class="withdraw-table" :empty-text="$t('funds.noData')">
      ...
      >
          <template #default="{row}">
            <span>{{ !isNaN(row.actualAmt)?row.actualAmt.toFixed(2):'-' }}</span>
          </template>
      </el-table-column> -->
      <el-table-column
...
const dateRangeOutpay = ref([]); //申请时间
const tableData = ref([]);
const total = ref(0);

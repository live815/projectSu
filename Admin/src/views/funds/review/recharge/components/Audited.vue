<template>
  <div class="funds-review-recharge-audited" v-loading="loading">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      <el-row :gutter="10">
        ...
        <el-col :span="4">
          <el-form-item :label="$t('funds.reviewer')" prop="reorderApprover" class="full-width">
            <el-input
              v-model="queryParams.reorderApprover"
              :placeholder="$t('funds.pleaseInput')"
              clearable
            />
          </el-form-item>
    ...
    <el-table :data="tableData" style="margin-top: 20px" :empty-text="$t('funds.noData')">
      ...
      >
        <template #default="{ row }">
          <el-button
            ...
            v-hasPermi="['withdrawl:order:approve']"
      ...
      >
        <template #default="{ row }">
          <span v-if="row.status === '3'">{{ $t('funds.reorderReviewReject') }}</span>
          <span v-if="row.status === '4'" style="color: #1890ff"
            >{{ $t('funds.reorderReviewPass') }}</span
          >
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          <template v-if="row.reorderApplicant  || row.reorderApplicantTime  || row.reorderApplicantMark">
            <div>
      ...
      <el-table-column
        :label="$t('funds.reorderReview')"
        align="center"
        width="240"
        :show-overflow-tooltip="true"
      >
        <template #default="{ row }">
          <template v-if="row.reorderApprover || row.reorderApprovalTime || row.reorderApprovalMark">
            <div>
              <span>{{ $t('funds.reviewer') }}</span>
              <span>{{ row.reorderApprover ? row.reorderApprover : "-" }}</span>
            </div>
            <div>
              <span>{{ $t('funds.reviewTime') }}</span>
              <span>{{ row.reorderApprovalTime ? row.reorderApprovalTime : "-" }}</span>
            </div>
            <div>
              <span>{{ $t('funds.reviewRemark') }}</span>
              <span>{{ row.reorderApprovalMark ? row.reorderApprovalMark : "-" }}</span>
            </div>
          </template>
          <span v-else>-</span>
        </template>
...
import { deepCopy,formatNumber } from "@/utils/index";
...
const queryParams = ref({
  ...
  approvalStartTime:null, //审核开始时间
  approvalEndTime:null, //审核结束时间
  pageNum: 1,
  pageSize: 10,
});
const statusArrList = ref([
  {
    label: i18n.global.t("funds.reorderReviewReject"),
    value: "3",
  },
  {
    label: i18n.global.t("funds.reorderReviewPass"),
    value: "4",
  },
]);
...
const dateRangeApproval = ref([]); //审核时间
const tableData = ref([]);
const total = ref(1);
...
function handleChangeApproval(){
  if ((dateRangeApproval.value && !dateRangeApproval.value.length) || dateRangeApproval.value == null) {
    queryParams.value.approvalStartTime = null;
    queryParams.value.approvalEndTime = null;
  } else {
    queryParams.value.approvalStartTime = dateRangeApproval.value[0];
    queryParams.value.approvalEndTime = dateRangeApproval.value[1];
  }
}
function getList() {
  loading.value = true;
...
onActivated(async()=>{
  !isFirstLoad.value && await getList();
})
</script>

<style lang="scss" scoped>
.funds-review-recharge-audited {
  // padding:0 20px;
  padding-bottom: 10px;
  .flex-refresh {
    display: flex;

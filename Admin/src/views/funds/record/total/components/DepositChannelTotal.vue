<template>
  <div class="deposit-channel-total-index">
...
<script setup>
import { getDepositListByDatetime } from "@/api/funds/record/index";
...
function getList(){
  return getDepositListByDatetime(queryParams.value).then(res=>{
    tableData.value = res || []
  })
}
...
onActivated(()=>{
  if(!isFirstLoad.value){
    handleQuery(btnList.value[currentIndex.value],currentIndex.value)
  }
})
</script>

<style lang="scss" scoped>
.deposit-channel-total-index {
  padding: 20px;
  .btn-group {
    margin-bottom: 16px;
    .el-button {

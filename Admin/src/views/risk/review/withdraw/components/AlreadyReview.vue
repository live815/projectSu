<template>
  <div class="already-review" v-loading="loading">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      <el-row :gutter="10">
        ...
        <el-col :span="4">
          <el-form-item :label="$t('risk.accountType')" prop="type" class="full-width">
            <el-select
        ...
        <el-col :span="4">
          <el-form-item :label="$t('risk.reviewer')" prop="adminName" class="full-width">
            <el-input
        ...
        <el-col :span="4">
          <el-form-item :label="$t('risk.operateClient')" prop="optClient" class="full-width">
            <el-select
        ...
        <el-col :span="4">
          <el-form-item :label="$t('risk.coinTypeQuote')" prop="currency" class="full-width">
            <el-select
        ...
        <el-col :span="4">
          <el-form-item :label="$t('risk.reviewStatus')" prop="status" class="full-width">
            <el-select
    ...
    <el-table :data="tableData" style="margin-top: 20px" :empty-text="$t('risk.noData')">
      ...
      >
        <template #default="{ row }">
          <el-button type="primary" size="small" @click="openReview(row)">{{
            $t('risk.check')
          }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('risk.userLevel')" align="center" width="180">
        <template #default="{ row }">
      ...
      >
        <template #default="{ row }">
          <span v-if="row.wdType==='0'">普通提现</span>
          <span v-else-if="row.wdType==='1'">大额提现</span>
          <span v-else>-</span>
        </template>
      </el-table-column> -->
      <el-table-column
        :label="$t('risk.operateClientNoQuote')"
        align="center"
        width="180"
        :show-overflow-tooltip="true"
      >
        <template #default="{ row }">
          <span>{{ filterLabel(clientList, Number(row.optClient)) }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          <template v-if="row.programNames">
            <el-tag class="tag-mr-4" type="success" v-for="item in row.programNames.split(',')">{{
              item
            }}</el-tag>
          </template>
          <span v-else>-</span>
      ...
      >
        <template #default="{ row }">
          <span>{{ filterLabel(riskStatusList, row.riskStatus) }}</span>
        </template>
      </el-table-column>
      <el-table-column
        :label="$t('risk.riskControlReview')"
        align="center"
        width="240"
        :show-overflow-tooltip="true"
      >
        <template #default="{ row }">
          <template v-if="row.riskApprover || row.riskApprovalTime || row.riskApprovalMark">
            <div>
              <span>{{ $t('risk.reviewer') }}</span>
              <span>{{ row.riskApprover ? row.riskApprover : '-' }}</span>
            </div>
            <div>
              <span>{{ $t('risk.reviewTime') }}</span>
              <span>{{ row.riskApprovalTime ? row.riskApprovalTime : '-' }}</span>
            </div>
            <div>
              <span>{{ $t('risk.reviewRemark') }}</span>
              <span>{{ row.riskApprovalMark ? row.riskApprovalMark : '-' }}</span>
            </div>
          </template>
          <span v-else>-</span>
        </template>
...
import { getWithdrawlOrderList, getMulReviewOrder } from '@/api/risk/review/index'
...
const queryParams = ref({
  ...
  orderNo: null, // 提现单号
  adminName: null, // 审核人
  optClient: null, // 操作端 string 0：未知 1：安卓 2：安卓H5 3：IOS 4：IOSH5 5：PC
  type: null, // 账号类型 string  0 普通 1 优质 2 关注
  riskTypes: [], // 风险类型
  financeTier: [], // 财务层级
...
const typeList = ref([
  {
    label: i18n.global.t('risk.normal'),
    value: '0'
  },
  {
    label: i18n.global.t('risk.excellent'),
    value: '1'
  },
  {
    label: i18n.global.t('risk.follow'),
    value: '2'
  }
])
...
const statusList = ref([
  ...
  {
    label: i18n.global.t('risk.riskControlReviewPass'),
    value: '3'
  },
  {
...
function handleExport() {
  proxy.download(
    ...
    `${i18n.global.t(
      'risk.reserveWithdrawAlreadyReviewList'

<template>
  <div class="review-on-hold-index" v-loading="loading">
      ...
      <el-row :gutter="10">
        ...
        <el-col :span="4">
          <el-form-item :label="$t('risk.reviewer')" prop="adminName" class="full-width">
            <el-input v-model="queryParams.adminName" :placeholder="$t('risk.pleaseInput')" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="4">
    ...
    :empty-text="$t('risk.noData')">
      ...
      >
        <template #default="{ row }">
          <span style="color: #f59a23">{{ $t('risk.riskControlHoldOn') }}</span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      >
        <template #default="{ row }">
          <template
            v-if="
              row.riskApprover ||
              row.riskApprovalTime ||
              row.riskApprovalMark
            "
          >
            <div>
              <span>{{$t('risk.reviewer')}}</span>
              <span>{{
                row.riskApprover ? row.riskApprover : "-"
              }}</span>
            </div>
            <div>
...
function handleExport() {
  proxy.download(
    ...
    `${i18n.global.t("risk.reserveWithdrawHoldOnList")}${new Date().toLocaleDateString()}.xlsx`
  );
}

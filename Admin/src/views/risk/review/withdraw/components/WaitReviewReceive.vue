<template>
  <div class="wait-review-receive-index" v-loading="loading">
      ...
      >
        <template #default="{ row }">
          <el-button
            ...
            >{{ $t('risk.recieve') }}</el-button
          >
          <el-button
            type="danger"
            size="small"
            @click="dispatch(row)"
            >{{ $t('risk.dispatch') }}</el-button
          >
        </template>
      </el-table-column>
      <el-table-column
...
import { getWithdrawlOrderList,dispatchOrder } from "@/api/risk/review/index";
...
const store = useUserStore(); //获取当前登录信息,store.userInfo.userName,store.userInfo.userId
...
const multiOperateList = ref([
  {
    label: i18n.global.t("risk.recieve"),
    value: "1",
  },
  {
    label: i18n.global.t("risk.dispatch"),
    value: "2",
  }
]);
const typeList = ref([
...
function multipleOperate() {
  ...
  //   open.value = true;
  // }
  if(multiOperate.value === "1"){
    dispatchOrder({
      orderNos:orderNos.value,
      riskUserName:store.userInfo.userName
    }).then(res=>{

<template>
  <div>
    <el-form label-width="90">
      <el-form-item :label="$t('agent.creditWallet')">
        <el-select
          v-model="detailDataForm.creditWalletStatus"
          :placeholder="$t('agent.pleaseSelect')"
        >
          <el-option :label="$t('agent.enable')" :value="0" />
          <el-option :label="$t('agent.disable')" :value="1" />
        </el-select>
      </el-form-item>
...
const submit = () => {
  formData.value.adjustContentType = 5;
  formData.value.agentId = route.query.id;
  formData.value.agentName = props.detailData.agentUsername;
  formData.value.adjustBefore = selectMap[props.detailData.creditWalletStatus];
  formData.value.adjustAfter =
    selectMap[detailDataForm.value.creditWalletStatus];
...
const selectMap = {
  0: i18n.global.t("agent.enable"),
  1: i18n.global.t("agent.disable"),
};
</script>

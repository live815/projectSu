<!-- 代理信息调整{{$t('agent.review')}} 待{{$t('agent.review')}} -->
<template>
  <div class="app-container">
    ...
    <el-table :data="agentAdjustmentList" style="width: 100%" :empty-text="$t('member.noData')">
      <el-table-column :label="$t('agent.handle')" width="230" align="center">
      ...
      <el-table-column
        property="adjustBefore"
        :label="$t('agent.adjustBefore')"
        width="180"
        align="center"
      >
      ...
      <el-table-column
        property="adjustAfter"
        :label="$t('agent.adjustAfter')"
        width="200"
        align="center"
      >
        <template #default="scope">
      ...
      <el-table-column property="status" :label="$t('agent.orderStatus')" width="230">
        <template #default="{ row }">
          <span style="color: #04f21c" v-if="row.status == 0">{{ $t('agent.waitReview') }}</span>
        </template>
      </el-table-column>

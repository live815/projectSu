  ...
  <div class="app-container">
    ...
    <el-table :data="fundsDataList" style="width: 100%">
      ...
      <el-table-column property="agentUsername" :label="$t('agent.agentInformation')" width="180">
        <template #default="{ row }">
          <div style="margin: 0; padding: 0">
            ...
            <p class="margin">
              {{ $t('agent.parentAccountQuote') }}{{ row.superiorAgentName || '--' }}
            </p>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column property="agentMode" :label="$t('agent.orderStatus')" width="180">
        <template #default="{ row }">
          <span class="yellow">{{ statusMap[row.status] }}</span>
      ...
      <el-table-column property="status" :label="$t('agent.billingMessage')" width="250">
        <template #default="{ row }">
          ...
          <div v-else style="margin: 0; padding: 0">
            <p class="margin">
              {{ $t('agent.transactionAgreement') }}{{ row.tradeProtocol == 1 ? 'TRC20' : 'ERC20' }}
            </p>
    ...
    <el-dialog v-model="isShowReview" title="" width="1200px" :center="true">
      <RiskControlReview
        v-if="isShowReview"
        :rowItem="rowItem"
        :status="2"

<template>
	<div class="popup-result">
		<p class="title">{{ $t('common.recentlyFiveTimesImplement') }}</p>
		<ul class="popup-result-scroll">
			<template v-if='isShow'>
				<li v-for='item in resultList' :key="item">{{item}}</li>
			</template>
			<li v-else>{{ $t('common.calculateResult') }}</li>
		</ul>
	</div>
</template>
...
function expressionChange() {
    ...
    if (resultArr.length === 0) {
        resultList.value = [i18n.global.t('common.resultIsNotReachCondition')];
    } else {
        resultList.value = resultArr;
        if (resultArr.length !== 5) {
            resultList.value.push(i18n.global.t('common.recentlyOneHUndredYears') + resultArr.length + i18n.global.t('common.totalResults'))
        }
    }
    // 计算完成-显示结果
    isShow.value = true;

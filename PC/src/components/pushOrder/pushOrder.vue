<template>
  <div class="swiperPushO">
    <div
      class="leftBtn"
      @click="prevGroup"
      v-show="liveStore.liveRoom.orderList && liveStore.liveRoom.orderList.length > 5"
    >
    ...
    <div class="swiper-swiperPushO">
      <div class="swiper-wrapper">
        ...
        <div
          v-for="(item, index) in liveStore.liveRoom.orderList"
          :key="index"
          @click="slideClicked(index, item)"
          :class="{ 'swiper-slide': true }"
        >
          <div class="order-info">
            <div class="pushOTop">{{$t('Components.GoldMedalTutor')}}</div>
            <div class="pushMain">
              ...
              <div class="pushOBtn" @click.stop="pushOdds(item)">{{$t('Components.FollowOrders')}}</div>
            </div>
          </div>
        </div>
      </div>
...
onMounted(() => {
  ...
  mySwiperPush = new Swiper('.swiper-swiperPushO', {
    ...
    grabCursor: liveStore.liveRoom.orderList.length > 5 ? true : false //光标显示手指滑动
  })
